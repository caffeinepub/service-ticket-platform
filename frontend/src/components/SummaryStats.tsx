...
import { Ticket, TicketStatus } from '../backend';
...
export function SummaryStats({ tickets }: SummaryStatsProps) {
    ...
    return (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {stats.map(stat => (
                <Card key={stat.label} className={`border ${stat.borderClass} shadow-card`}>
                    <CardContent className="p-4">
                        <div className="flex items-center justify-between mb-2">
                            <span className="text-sm font-medium text-muted-foreground">{stat.label}</span>
                            <span className={`${stat.colorClass} ${stat.bgClass} p-1.5 rounded-md`}>
                                {stat.icon}
                            </span>
                        </div>
                        <p className={`text-3xl font-bold font-display ${stat.colorClass}`}>{stat.count}</p>
                    </CardContent>
                </Card>
            ))}
        </div>
