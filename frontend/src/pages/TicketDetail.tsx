...
import { Ticket, TicketStatus, UserRole } from '../backend';
import { useComments, useAddComment, useUpdateTicketStatus } from '../hooks/useQueries';
...
export function TicketDetail({ ticket, onBack }: TicketDetailProps) {
    ...
    const { data: comments, isLoading: commentsLoading, refetch: refetchComments } = useComments(ticket.id);
    ...
    const handleAddComment = async () => {
        ...
        setCommentError('');
        try {
            await addComment.mutateAsync({ ticketId: ticket.id, message: commentText.trim() });
            setCommentText('');
        } catch {
                    ...
                    <Card className="shadow-card border border-border">
                        <CardHeader className="pb-3">
                            <div className="flex items-center justify-between">
                                ...
                                <Button variant="ghost" size="sm" onClick={() => refetchComments()} className="h-7 w-7 p-0">
                ...
                <div className="space-y-4">
                    <Card className="shadow-card border border-border">
                        ...
                        <CardContent className="space-y-4">
                            ...
                            <div>
                                <p className="text-xs text-muted-foreground mb-1">Module Name</p>
                        ...
                        <Card className="shadow-card border border-border">
                            <CardContent className="pt-4 pb-4">
                                <p className="text-sm text-muted-foreground text-center">
                                    This ticket is {STATUS_LABELS[ticket.status].toLowerCase()} and cannot be updated further.
                                </p>
                            </CardContent>
                        </Card>
                    )}
